...
"""
        ...
        if self.simple_mode_var.get():
            ...
            for item in self.button_layout:
                ...
                if (
                    ...
                    and action_id in simple_mode_ids
                    and action_id not in picked_ids
                ):
                    # 精简模式下，忽略文件相关按钮按实际文件状态强制保留；
                    # 其它按钮仍尊重用户自己的显示/隐藏设置。
                    force_show_ignore_button = action_id in {"edit_ignore", "generate_ignore"}

                    if force_show_ignore_button or item.get("visible", True):
                        visible_items.append(item)
                        picked_ids.add(action_id)

            visible_items.sort(key=lambda item: simple_mode_ids.index(item.get("id")))
        else:
            visible_items = [
    ...
    def toggle_simple_mode(self):
        if self.simple_mode_var.get():
            self.status_text.set("已切换到精简模式：保留生成忽略文件、一键上传、状态、日志和恢复相关按钮")
        else:
            self.status_text.set("已切换到标准模式")
    ...
    def choose_simple_upload_files(self, repo):
        ...
        return ["add", "--"] + relative_paths, "添加部分文件"
    ...
    def git_one_click_upload_to_github(self):
        ...
        def task():
            ...
            all_output.append(f"\n--- {add_title} ---\n")
            all_output.append("git " + " ".join(add_args) + "\n")
            add_code, add_output = self.run_command_with_code(["git"] + add_args, repo)
            all_output.append(add_output)

            if add_code != 0:
                self.after(0, lambda: self.append_output("".join(all_output)))
                self.after(0, lambda: self.handle_common_git_problem(add_output, add_title))
                self.after(0, lambda: self.status_text.set(f"一键上传失败：{add_title}失败"))
                return

            self.after(0, lambda: self.status_text.set("添加文件成功。下一步：添加说明并提交"))

            all_output.append("\n--- 添加说明并提交 ---\n")
            all_output.append(f"git commit -m {msg}\n")
            commit_code, commit_output = self.run_command_with_code(["git", "commit", "-m", msg], repo)
            all_output.append(commit_output)

            if commit_code != 0:
                self.after(0, lambda: self.append_output("".join(all_output)))
                self.after(0, lambda: self.handle_common_git_problem(commit_output, "添加说明并提交"))
            ...
            if push_code == 0:
                ...
                self.after(0, lambda: self.handle_push_failure(repo, branch, push_output))
