...
"""
    ...
    def finish_one_click_add_and_choose_commit_mode(self, repo, add_title, add_code, add_output):
        ...
        self.run_one_click_commit_and_push(repo, commit_commands)

    def run_one_click_commit_and_push(self, repo, commit_commands):
    ...
    def build_separate_commit_commands(self, staged_statuses, cancel_status_text):
        ...
        if deleted_files:
            # 删除文件不再要求用户逐个填写说明，统一自动提交。
            commands.append((
                ["commit", "-m", "删除旧文件", "--"] + deleted_files,
                "提交删除文件"
            ))
